...
|---------------------------------------------------------------
| Pagina en donde se encuentra el javascript de la pagina FRM_Fusion.php
*/
$(document).ready(function(){
    ...
    /*Sentencia para validar los campos del formulario FRM_Fusion*/
    $("#Fusion").valida();
    /* Pestaña de Derecha */
    $(".tab_content").hide();
    ...
    /* Sentencia para Cargar el Grid de las Fusions */
    cargagrid_Fusion(accion);
...
/* Funcion que carga los datos de la Fusion seleccionada al formulario */
function MostrarDatos(cod, page){
    $.getJSON("Planificacion_Produccion/Servicios/Proceso_Fusion/MAN_Fusion.php?m=1&id="+cod+"&pag="+page, function(data){
...
function CargaTab2(activeTab, accion){
    var cod='';
    cod = $("#tblFusion").jqGrid('getGridParam','selrow');
    if(cod != '' && cod != null){
        Desabilitar();
        $(".tab_content").hide();
...
/* Funcion para Editar una Fusion Seleccionada del grid */
...
/* Funcion para Recarga el Grid de las Fusions */
function ReloadGrid(){
    jQuery("#tblFusion").jqGrid('setGridParam',
    {
        url:'Planificacion_Produccion/Servicios/Proceso_Fusion/Tabla/TAB_Fusion.php'
    }).trigger("reloadGrid");
}
/* Funcion para habilitar los campos del formulario*/
function Habilitar(){
    $('#Fusion input').removeAttr('readonly');
    $('#Fusion input').removeAttr('disabled');
    $('#Fusion select').removeAttr('selected');
    $('#Fusion select').removeAttr('disabled');
    $("#txt_cod").attr('readonly','readonly');
}
/* Funcion para Desabilitar los campos del formulario*/
function Desabilitar(){
    $('#Fusion input').attr('readonly','readonly');
    $('#Fusion input[type="radio"]').attr('disabled','disabled');
    $('#Fusion select').attr('disabled','disabled');
}
/* Funcion para eliminar una de las Fusions de la fila del Grid seleccionado */
function fun_del(cod){
    message('Proceso de Fusión','warning','¿Está seguro de eliminar el Proceso de Fusión?','warning_aceptar',"'"+cod+"'",'messageclose()');
}
/* Funcion para eliminar las Fusions seleccionadas del menu del grid */
function fun_del2(){
    var cod = $("#tblFusion").jqGrid('getGridParam','selarrrow');
    if(cod != ''){
        message('Proceso de Fusión','question','¿Está seguro de eliminar el Proceso de Fusión?','question_aceptar',"'"+cod+"'",'messageclose()');
    }
}
/* Funcion para eliminar las Fusions Seleccionadas del menu del grid */
function fun_del3(){
...
function question_aceptar(cod){
    var codpar = cod+',';
    var accion = $("#sp_accion").html();
    $.post('Planificacion_Produccion/Servicios/Proceso_Fusion/MAN_Fusion.php',{
        del:1,
        cod:codpar
    },function(){
        ReloadGrid();
...
function quest_aceptar(){
    var form = $("#Fusion").serialize();
    $.ajax({
        type: "POST",
        url: "Planificacion_Produccion/Servicios/Proceso_Fusion/MAN_Fusion.php",
        data: form+'&a=1',
        success: function(data) {
            var arr = data.split('::');
            if(arr[0]=='0'){
                message('Proceso de Fusión','error', 'Los campos no deben estar vacios', 'messageclose_error',"'"+arr[1]+"'", '');
            }else{
                message('Proceso de Fusión','info',arr[1],'info_aceptar','','');
            }
        }
    });
    messageclose();
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if(arr[0]=='0'){
                ...
                message('Proceso de Fusión','info',arr[1],'info_aceptarnuevo','','');
            }
        }
    });
    messageclose();
...
function fun_save(accion){
    message('Proceso de Fusión','question','Está seguro de grabar el nuevo Registro','quest_aceptar','','messageclose()');
}
...
function fun_saveandnew(){
    message('Proceso de Fusión', 'question', 'Esta seguro de grabar el nuevo Registro' , 'quest_aceptarnuevo','','messageclose()');
}
...
/* Funcion para Listar el Grid de las Fusions */
function cargagrid_Fusion(accion){
    var permisos = $("#sp_accion").html();
    ListaAccionDet(permisos,'EditDel');
    jQuery("#tblFusion").jqGrid({
        url:'Planificacion_Produccion/Servicios/Proceso_Fusion/Tabla/TAB_Fusion.php',
        datatype: "json",
        ...
        gridComplete: function(){
            ...
            var ids = $("#tblFusion").jqGrid('getDataIDs');
            ...
            for(var i=0;i < ids.length;i++){
                ...
                $("#tblFusion").jqGrid('setRowData',ids[i],{
                    botones: edit+" "+dele
                });
            }
            ...
            /* Sección en donde se le coloca dos barra de herramientas al jqGrid.
                            ...
                            * top => En caso se desee colocar en la Fusion superior. */
            $("#t_tblFusion").append("<div>&nbsp; Agrupar por: <select id='cbo_columns' style='width: 120px;'><option value='clear'>&nbsp;Ninguna</option><option value='par_in11_cod'>&nbsp;Código</option><option value='par_vc50_desc'>&nbsp;Descripción</option></select></div>");
            $("#t_tblFusion").attr('style','width:885px; margin-left:-1px;');
            ...
            $("#cbo_columns").change(function(){
                ...
                if(vl){
                    if(vl == "clear"){
                        $("#tblFusion").jqGrid('groupingRemove',true);
                    }else{
                        $("#tblFusion").jqGrid('groupingGroupBy',vl);
                    }
                }
            });
        }
    ...
    $("#tblFusion").jqGrid('hideCol',["condicion"]);
    /* Ordenando los checkbox */
    $("#cb_tblFusion").attr('style','margin-left:4px; margin-top:2px;');
    /* Se agrega al funcionalidad de multiples busquedas y el refrescado del grid */
    $("#tblFusion").jqGrid('navGrid','#PagFusion',{
        add:false,
        edit:false,
        del:false,
        refresh:true
