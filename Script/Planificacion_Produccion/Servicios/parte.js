...
|---------------------------------------------------------------
| Pagina en donde se encuentra el javascript de la pagina FRM_Parte.php
*/
$(document).ready(function(){
    ...
    /*Sentencia para validar los campos del formulario FRM_Parte*/
    $("#Parte").valida();
    /* Pestaña de Derecha */
    $(".tab_content").hide();
    ...
    /* Sentencia para Cargar el Grid de las Partes */
    cargagrid_parte(accion);
...
/* Funcion que carga los datos de la parte seleccionada al formulario */
function MostrarDatos(cod, page){
    $.getJSON("Planificacion_Produccion/Servicios/Parte/MAN_Parte.php?m=1&id="+cod+"&pag="+page, function(data){
...
function CargaTab2(activeTab, accion){
    var cod='';
    cod = $("#tblParte").jqGrid('getGridParam','selrow');
    if(cod != '' && cod != null){
        Desabilitar();
        $(".tab_content").hide();
...
function fun_new(accion){
    ...
    $("#tabs-2").fadeIn();
    $("#txt_part_desc").focus();
    $("#txt_part_cod2").attr("style","display:none");
    $("#txt_part_cod").attr("style","display:none");
}
/* Funcion para Editar una Parte Seleccionada del grid */
function fun_edi(cod, accion){
    ...
    $("#txt_part_desc").focus();
    $("#txt_part_cod2").attr("style","width: 155px");
    $("#txt_part_cod").attr("style","width: 200px");
}
...
/* Funcion para Recarga el Grid de las Partes */
function ReloadGrid(){
    jQuery("#tblParte").jqGrid('setGridParam',
    {
        url:'Planificacion_Produccion/Servicios/Parte/Tabla/TAB_Parte.php'
    }).trigger("reloadGrid");
}
/* Funcion para habilitar los campos del formulario*/
function Habilitar(){
    $('#Parte input').removeAttr('readonly');
    $('#Parte input').removeAttr('disabled');
    $('#Parte select').removeAttr('selected');
    $('#Parte select').removeAttr('disabled');
    $("#txt_part_cod").attr('readonly','readonly');
}
/* Funcion para Desabilitar los campos del formulario*/
function Desabilitar(){
    $('#Parte input').attr('readonly','readonly');
    $('#Parte input[type="radio"]').attr('disabled','disabled');
    $('#Parte select').attr('disabled','disabled');
}
/* Funcion para eliminar una de las Partes de la fila del Grid seleccionado */
function fun_del(cod){
    message('Partes','warning','¿Está seguro de eliminar la Parte?','warning_aceptar',"'"+cod+"'",'messageclose()');
}
/* Funcion para eliminar las Partes seleccionadas del menu del grid */
function fun_del2(){
    var cod = $("#tblParte").jqGrid('getGridParam','selarrrow');
    if(cod != ''){
        message('Partes','question','¿Está seguro de eliminar Las Partes?','question_aceptar',"'"+cod+"'",'messageclose()');
    }
}
/* Funcion para eliminar las Partes Seleccionadas del menu del grid */
function fun_del3(){
...
function question_aceptar(cod){
    var codpar = cod+',';
    var accion = $("#sp_accion").html();
    $.post('Planificacion_Produccion/Servicios/Parte/MAN_Parte.php',{
        del:1,
        cod:codpar
    },function(){
        ReloadGrid();
...
function quest_aceptar(){
    var form = $("#Parte").serialize();
    $.ajax({
        type: "POST",
        url: "Planificacion_Produccion/Servicios/Parte/MAN_Parte.php",
        data: form+'&a=1',
        success: function(data) {
            var arr = data.split('::');
            if(arr[0]=='0'){
                message('Parte','error', 'Los campos no deben estar vacios', 'messageclose_error', "'"+arr[1]+"'", '');
            }else{
                message('Parte','info',arr[1],'info_aceptar','','');
            }
        }
    });
    messageclose();
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if(arr[0]=='0'){
                ...
                message('Parte','info',arr[1],'info_aceptarnuevo','','');
            }
        }
    });
    messageclose();
...
function fun_save(accion){
    message('Partes','question','Está seguro de grabar el nuevo Registro','quest_aceptar','','messageclose()');
}
...
function fun_saveandnew(){
    message('Parte', 'question', 'Esta seguro de grabar el nuevo Registro' , 'quest_aceptarnuevo','','messageclose()');
}
...
/* Funcion para Listar el Grid de las Partes */
function cargagrid_parte(accion){
    var permisos = $("#sp_accion").html();
    ListaAccionDet(permisos,'EditDel');
    jQuery("#tblParte").jqGrid({
        url:'Planificacion_Produccion/Servicios/Parte/Tabla/TAB_Parte.php',
        datatype: "json",
        colNames:['','Código','Descripción','Alias','Tipo'],
        colModel:[
        {
            name: 'botones',
            index: 'botones',
        ...
        gridComplete: function(){
            ...
            var ids = $("#tblParte").jqGrid('getDataIDs');
            ...
            for(var i=0;i < ids.length;i++){
                ...
                $("#tblParte").jqGrid('setRowData',ids[i],{
                    botones: edit+" "+dele
                });
            }
            ...
            $("#t_tblParte").append("<div>&nbsp; Agrupar por: <select id='cbo_columns' style='width: 120px;'><option value='clear'>&nbsp;Ninguna</option><option value='par_in11_cod'>&nbsp;Código</option><option value='par_vc50_desc'>&nbsp;Descripción</option></select></div>");
            $("#t_tblParte").attr('style','width:885px; margin-left:-1px;');
            ...
            $("#cbo_columns").change(function(){
                ...
                if(vl){
                    if(vl == "clear"){
                        $("#tblParte").jqGrid('groupingRemove',true);
                    }else{
                        $("#tblParte").jqGrid('groupingGroupBy',vl);
                    }
                }
            });
        }
    ...
    $("#tblParte").jqGrid('hideCol',["condicion"]);
    /* Ordenando los checkbox */
    $("#cb_tblParte").attr('style','margin-left:4px; margin-top:2px;');
    /* Se agrega al funcionalidad de multiples busquedas y el refrescado del grid */
    $("#tblParte").jqGrid('navGrid','#PagParte',{
        add:false,
        edit:false,
        del:false,
        refresh:true
