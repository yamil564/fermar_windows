...
|---------------------------------------------------------------
| Pagina en donde se encuentra el javascript de la pagina FRM_ingresoCalDet.php
 */
            ...
            if(dniSuper != '' && dniSuper.length > 7){
                ...
                $.post('MAN_PDA.php',{
                    ...
                    if(data == codSuper){
                        ...
                        $.post('MAN_PDA.php',{
                            ot:ot,var1:0,var2:0,pro:pro,core:core,saveItemCaliFinal:1,codItem:codItem,codSuper:codSuper
                        }, function(data){
                            var save = data.split('::');
                            if(save[0] == '0'){
                        ...
                        alert('código del Supervisor invalido');$("#txt_cal_super").removeAttr("readonly");$('#txt_cal_super').val('');$('#txt_cal_super').focus();
                    }
                });
            }else{
                alert("Ingrese un código valido del Supervisor.");$("#txt_prod_codOpe").val('');$("#txt_prod_codOpe").focus();
            }
        }else{
...
function enter(e) {
    if(e.keyCode==13){
        if($("#txt_procesoID").val() != '' && $("#txt_prod_items").val() != ''){
            ...
            //Preguntando si el Item tiene un acabado diferente a acero natural
            $.post('MAN_PDA.php',{
                valTipAcab:1,cod:cod
            },function(data){
                if(data != 'A001'){      
                    ...
                    alert("El Item no requiere de este tipo de proceso de calidad.");limpiar();
                }
            });            
        }else{
            alert("Seleccione un proceso o ingrese un código");limpiar();
        }
    }
}
