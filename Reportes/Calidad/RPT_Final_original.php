...
class CLS_CalFinal1 extends PDF_addonXMP {
    ...
    function setData($deg, $cliente, $ots, $detalle, $fechaper, $reg, $proyecto, $f1, $f2) {
        $this->deg = $deg;
    ...
    function Header() {
        ...
        $this->SetFont('Arial', 'B', 9);
        $this->SetTextColor(23);
        $this->MultiCell(70, 4, utf8_decode('SISTEMA DE GESTIÓN DE LA CALIDAD'), '', 'C', '', false);

        $this->SetY(14);
        $this->SetX(86);
        $this->SetFont('Arial', 'B', 7);
        $this->SetTextColor(23);
        $this->MultiCell(60, 4, utf8_decode('REGISTRO DE LIBERACIÓN FINAL'), '', 'C', '', false);

        $this->SetY(7);
        $this->SetX(176);
        $this->SetFont('Arial', 'B', 6);
        $this->SetTextColor(23);
        $this->MultiCell(25, 4, utf8_decode('Codigo     :  F-CC-005'), '', 'L', '', false);

        $this->SetY(10);
        $this->SetX(176);
        $this->SetFont('Arial', 'B', 6);
        $this->SetTextColor(23);
        $this->MultiCell(25, 4, utf8_decode('Versión     : 01'), '', 'L', '', false);

        $this->SetY(13);
        $this->SetX(176);
        $this->SetFont('Arial', 'B', 6);
        $this->SetTextColor(23);
        $this->MultiCell(28, 4, utf8_decode('Aprobado :  01/02/2011'), '0', 'L', '', false);
        $this->Cabezera(0);
    }
...
#DATOS GENERALES - Cliente
        ...
        $this->SetFont('Arial', 'B', 8);
        $this->SetTextColor(125);
        $this->MultiCell(130, 4, utf8_decode('Cliente: ') . $this->cliente, '1', 'L', false);
...
#DATOS GENERALES - OT
        ...
        $this->SetFont('Arial', 'B', 8);
        $this->SetTextColor(125);
        $this->MultiCell(57, 4, utf8_decode('OT: ') . $this->ots, '1', 'L', false);
...
#DATOS GENERALES - Designación
        ...
        $this->SetFont('Arial', 'B', 8);
        $this->SetTextColor(125);
        $this->MultiCell(130, 4, utf8_decode('Locación: PLANTA FERMAR - VILLA EL SALVADOR'), '1', 'L', false);

#DATOS GENERALES - BLANCO
        $this->SetY($this->GetY() - 4);
        $this->SetX($posX + 146);
        $this->SetFont('Arial', 'B', 8);
        $this->SetTextColor(125);
        $this->MultiCell(57, 4, utf8_decode('Fecha de Inspección: ') . $this->fechaper, '1', 'L', false);
...
#DATOS GENERALES - Proyecto
        ...
        $this->SetFont('Arial', 'B', 8);
        $this->SetTextColor(125);
        $this->MultiCell(187, 4, utf8_decode('Proyecto : ') . $this->proyecto, '1', 'L', false);
...
#DESCRIPCIÓN - Marca
        ...
        $this->SetFont('Arial', 'B', 8);
        $this->SetTextColor(23);
        $this->MultiCell(53, 8, utf8_decode('Marca'), '1', 'C', false);
...
#DESCRIPCIÓN - Serie
        ...
        $this->SetFont('Arial', 'B', 8);
        $this->SetTextColor(23);
        $this->MultiCell(12, 8, utf8_decode('Serie'), '1', 'C', false);
...
#DESCRIPCIÓN - Ancho
        ...
        $this->SetFont('Arial', 'B', 7);
        $this->SetTextColor(23);
        $this->MultiCell(25, 8, utf8_decode('Insp. Dimencional'), '1', 'C', false);
...
#DESCRIPCIÓN - Inspeccion SOLD
        ...
        $this->SetFont('Arial', 'B', 7);
        $this->SetTextColor(23);
        $this->MultiCell(28, 4, utf8_decode('Insp. Visual'), 'LRT', 'C', false);
...
#DESCRIPCIÓN - Inspeccion SOLD
        ...
        $this->SetFont('Arial', 'B', 7);
        $this->SetTextColor(23);
        $this->MultiCell(28, 4, utf8_decode('de Soldadura'), 'LRB', 'C', false);
...
#DESCRIPCIÓN - OPERARIO
        ...
        $this->SetFont('Arial', 'B', 7);
        $this->SetTextColor(23);
        $this->MultiCell(28, 4, utf8_decode('Insp. de Limpieza'), 'LRT', 'C', false);
...
#DESCRIPCIÓN - OPERARIO
        ...
        $this->SetFont('Arial', 'B', 7);
        $this->SetTextColor(23);
        $this->MultiCell(28, 4, utf8_decode('y Acabado'), 'RLB', 'C', false);
...
#DESCRIPCIÓN - RESULTADO
        ...
        $this->SetFont('Arial', 'B', 8);
        $this->SetTextColor(23);
        $this->MultiCell(20, 8, utf8_decode('Resultado'), '1', 'C', false);
        $this->SetX(6);
       //#LINEAS
...
//       $this->SetFont('Arial', 'B', 8);
//       $this->SetTextColor(23);
//       $this->MultiCell(197, 176, utf8_decode(''), '1', 'L', false); 
    }
    ...
    function Footer() {
        ...
        $this->MultiCell(166, 4, utf8_decode("Conforme = C                                               No Conforme = NC                                               No Aplica = NA"), '0', 'C', false);
        ...
        $this->SetFont('Arial', '', 6);
        $this->SetTextColor(128);
        $this->MultiCell(166, 4, utf8_decode("Los materiales que tengan como resultado la designación 'C' conforme serán su despacho  y podrán pasar  al siguiente proceso. Los materiales que tengan cono resultado la designación 'NC' No Conforme se les abrirá un registro de No Conformidad para su tratamiento correspondiente."), '0', '0', false);

        $this->SetY($this->GetY() + 0);
        $this->SetX(16);
        $this->SetFont('Arial', 'B', 8);
        $this->SetTextColor(23);
        $this->MultiCell(187, 4, utf8_decode('5. Notas: Equipo empleado: Wincha de 8 mt.'), '1', 'L', '', false);
        ...
        $this->SetTextColor(128);
        if ($_REQUEST['fir'] == 1) {
            $this->MultiCell(55, 20, $this->Image('../../Images/' . $this->f1, 28, $this->GetY(), 30, 20, 'JPG', '', 0, false), 'LBR', '0', false);
        } else {
...
$db = new MySql();
$pdf = new CLS_CalFinal1();
$clsOT = new RPT_CalFin1();
$op = $_REQUEST['op'];$cLOTE=0;$cant=0;
$apertura = $clsOT->SP_LisCabezera($op);
$rowCabe2 = $clsOT->SP_LisCabezera2($op);
$rowFooter = $clsOT->SP_Firmas();
$pdf->setData(1, $rowCabe2['cli_vc20_razsocial'], $rowCabe2['ort_vc20_cod'], 1, $apertura, 1, $rowCabe2['pyt_vc150_nom'], $rowFooter['fir_vc30_f1'], $rowFooter['fir_vc30_f2']);
$pdf->AddPage();$pdf->AliasNbPages();
$sql = $clsOT->SP_LisFin1Cal($op);
$sql1 = $clsOT->SP_LisFin1Cal($op);//$pdf->SetY(-249);
//Sql para el corte de lso lotes en los reporte de calidad armado
$desOT = $rowCabe2['ort_vc20_cod'];
$row1 = $db->fetch_assoc($sql1);
$loteCorte = $row1['orc_in11_lote'];

//Mostrando el contenido del reporte
while($row = $db->fetch_assoc($sql)){
    ...
    if($loteCorte != $row['orc_in11_lote']){
        ...
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->SetTextColor(23);
        $pdf->MultiCell(53, 4, utf8_decode($desOT) . '  LOTE ' . $loteCorte, '1', 'C', '0', false);
    ...
    $pdf->SetX(16);
    $pdf->SetFont('Arial', '', 7);
    $pdf->SetWidths(array(11, 53, 12, 10, 25, 28, 28, 20)); // Coloca tamaño a las columnas a mostrar
    ...
    $pdf->Row(array(
        ...
        utf8_decode(1), utf8_decode('C'), utf8_decode('C'), utf8_decode('C'),
        utf8_decode('C')), 0, 1); // Columnas de la tabla a mostrar
        if($pdf->GetY() >= '213'){ $pdf->AddPage();$pdf->AliasNbPages(); }
$cant++;                 
}
