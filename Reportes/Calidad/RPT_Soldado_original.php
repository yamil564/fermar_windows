...
class CLS_CalSol extends PDF_addonXMP {
    ...
    function Header() {
        ...
        $this->SetTextColor(23);
        $this->MultiCell(60, 4, utf8_decode('REGISTRO DE INSPECIÓN DE SOLDADURA'), '', 'C', '', false);

        $this->SetY(7);
        $this->SetX(176);
        $this->SetFont('Arial', 'B', 6);
        $this->SetTextColor(23);
        $this->MultiCell(25, 4, utf8_decode('Codigo     :  F-CC-004'), '', 'L', '', false);
    ...
    function Footer() {
        ...
        $this->SetFont('Arial', 'B', 8);
        $this->SetTextColor(23);
        $this->MultiCell(187, 16, utf8_decode(''), 'LR', 'L', '', false);
...
#DATOS GENERALES - Designación
        ...
        $this->SetFont('Arial', 'B', 8);
        $this->SetTextColor(125);
        $this->MultiCell(60, 4, utf8_decode('Espec. Técnica: ') . $this->deg, '1', 'L', false);
...
#DATOS GENERALES - BLANCO
        ...
        $this->SetFont('Arial', 'B', 8);
        $this->SetTextColor(125);
        $this->MultiCell(67, 4, utf8_decode(''), 'BT', 'L', false);
...
#DESCRIPCIÓN - ITEM
        ...
        $this->SetFont('Arial', 'B', 8);
        $this->SetTextColor(23);
        $this->MultiCell(9, 8, utf8_decode('Item'), '1', 'C', false);
...
#DESCRIPCIÓN - Marca
        ...
        $this->SetFont('Arial', 'B', 8);
        $this->SetTextColor(23);
        $this->MultiCell(40, 8, utf8_decode('Marca'), '1', 'C', false);
...
#DESCRIPCIÓN - Ancho
        ...
        $this->SetFont('Arial', 'B', 7);
        $this->SetTextColor(23);
        $this->MultiCell(17, 4, utf8_decode('Tamaño de'), 'TRL', 'C', false);
...
#DESCRIPCIÓN - Ancho
        ...
        $this->SetFont('Arial', 'B', 7);
        $this->SetTextColor(23);
        $this->MultiCell(17, 4, utf8_decode('Soldadura'), 'RLB', 'C', false);
...
#DESCRIPCIÓN - Inspeccion SOLD
        ...
        $this->SetFont('Arial', 'B', 7);
        $this->SetTextColor(23);
        $this->MultiCell(16, 4, utf8_decode('Inspección'), 'TRL', 'C', false);
...
#DESCRIPCIÓN - Inspeccion SOLD
        ...
        $this->SetFont('Arial', 'B', 7);
        $this->SetTextColor(23);
        $this->MultiCell(16, 4, utf8_decode('Visual'), 'RLB', 'C', false);
...
#DESCRIPCIÓN - OPERARIO
        ...
        $this->SetFont('Arial', 'B', 8);
        $this->SetTextColor(23);
        $this->MultiCell(59, 8, utf8_decode('Operario'), '1', 'C', false);
...
#DESCRIPCIÓN - OPERARIO
        ...
        $this->SetFont('Arial', 'B', 8);
        $this->SetTextColor(23);
        $this->MultiCell(16, 4, utf8_decode('Fecha de'), 'LRT', 'C', false);
...
#DESCRIPCIÓN - OPERARIO
        ...
        $this->SetFont('Arial', 'B', 8);
        $this->SetTextColor(23);
        $this->MultiCell(16, 4, utf8_decode('Insp.'), 'BRL', 'C', false);
...
$db = new MySql();
$pdf = new CLS_CalSol();
$clsOT = new RPT_CalSol();//ort_vc50_sSol
$op = $_REQUEST['op'];$contador=-1;$cant=0;
$rowsCabe = $clsOT->SP_LisCabezera2($op);
$apertura = $clsOT->SP_LisCabezera($op);
$sqlFir = $clsOT->SP_Firmas();
$pdf->setData($rowsCabe['ort_vc50_sSol'], $rowsCabe['cli_vc20_razsocial'],$rowsCabe['ort_vc20_cod'], $rowsCabe['mat_vc50_desc'] .' - '. $rowsCabe['fierro'], $apertura, 6,  $sqlFir['fir_vc30_f1'], $sqlFir['fir_vc30_f2']);
$pdf->AddPage();$pdf->AliasNbPages();
$sql1 = $clsOT->LisSolCal($op);
$sql = $clsOT->LisSolCal($op);

//Sql para el corte de lso lotes en los reporte de calidad armado
$desOT = $rowsCabe['ort_vc20_cod'];
$row1 = $db->fetch_assoc($sql1);
$loteCorte = $row1['orc_in11_lote'];

//Mostrando el cuerpo del reporte
//$pdf->SetY(-249);
...
$contador++;
     ...
     if($loteCorte != $row['orc_in11_lote']){
        ...
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->SetTextColor(23);
        $pdf->MultiCell(40, 4, utf8_decode($desOT) . '  LOTE ' . $loteCorte, '1', 'C', '0', false);
...
$pdf->SetX(16);
$pdf->SetFont('Arial', '', 7);
$pdf->SetWidths(array(9, 40, 9, 10, 17, 16, 59, 16, 11)); // Coloca tamaño a las columnas a mostrar
...
$pdf->Row(array(utf8_decode($row['dic_in11_items']), utf8_decode($row['con_vc20_marcli']), utf8_decode($row['orc_in11_serie']),
                utf8_decode(1), utf8_decode('C'), utf8_decode('C'), utf8_decode($row['nombre']), utf8_decode($row['fecha']),
                utf8_decode('C')), 0, 1); // Columnas de la tabla a mostrar
                if($pdf->GetY() > '215'){ $pdf->AddPage();$pdf->AliasNbPages(); }
$cant++;                 
}
