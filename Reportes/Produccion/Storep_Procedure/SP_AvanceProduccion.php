 ...
 * Modificado por: Fernando Cuesta Barreto
 * Fecha de la ultima modificacion:15/10/2014
 * Se agrega los siguientes campos en el reporte diario avanzado: Tipo rejilla,
 *  largo, ancho, área y peso.
 *  
  |---------------------------------------------------------------
...
function fun_colmProc($proceso) {
        ...
        switch ($proceso) {
            ...
            case 11: $colm = 'dot_do_plib1';break;
        }
        return $colm;
}
...
class RPT_RegDiario {
    ...
    function SP_ListProd($op, $proc) {
        ...
        if($proc != 14 && $proc != 15){
            ...
            $cons = $db->consulta("SELECT dic_in11_items, dip.ort_vc20_cod, orc_in11_lote, orc_vc20_marclis, CONCAT(tra_vc150_ape,', ',tra_vc150_nom) AS nombre, pro_vc10_alias
                               FROM detalle_inspeccion_calidad dip, orden_conjunto orc, trabajador tra, orden_produccion orp, proceso pro
                               WHERE dip.orc_in11_cod=orc.orc_in11_cod AND dip.tra_in11_ope=tra.tra_in11_cod AND pro.pro_in11_cod=dip.pro_in11_cod AND 
                               dip.ort_vc20_cod=orp.ort_vc20_cod AND dip.pro_in11_cod = '$proc' AND orp.orp_in11_numope = '$op'
                               ORDER BY dic_in11_items ASC");
        }
        return $cons;
    }
    ...
    function SP_LisSUpervisor($op, $proc) {
        ...
        if($proc != 14 && $proc != 15){
            ...
            ($proc == '') ? $orden = "dip.pro_in11_cod ASC, dic_in11_items ASC" : $orden = "dic_in11_items ASC";
        }
        ...
        if($proc != 14 && $proc != 15){
            $cons = $db->consulta("SELECT CONCAT(tra_vc150_ape,', ',tra_vc150_nom) AS super FROM detalle_inspeccion_prod dip, orden_conjunto orc,
                ...
                $cons = $db->consulta("SELECT CONCAT(tra_vc150_ape,', ',tra_vc150_nom) AS super FROM detalle_inspeccion_calidad dip, orden_conjunto orc,
                                trabajador tra, orden_produccion orp, proceso pro WHERE dip.orc_in11_cod=orc.orc_in11_cod AND 
                                dip.tra_in11_sup=tra.tra_in11_cod AND pro.pro_in11_cod=dip.pro_in11_cod AND  dip.ort_vc20_cod=orp.ort_vc20_cod 
                                AND $condicion ORDER BY $orden");
            }
...
// * ancho, área y peso.
    ...
    function SP_ListProd($op, $proc) {
        ...
        if($proc != 14 && $proc != 15){
            ...
            $cons = $db->consulta("SELECT dic_in11_items, dip.ort_vc20_cod, orc_in11_lote, orc_vc20_marclis, pro_vc10_alias,conj.cob_vc50_cod,con_do_largo,con_do_ancho,con_do_pestotal,con_do_areaTotal,
            (SELECT CONCAT(tra_vc150_ape,', ',tra_vc150_nom) FROM trabajador tra WHERE tra.tra_in11_cod=dip.tra_in11_ope) AS operario,
            (SELECT CONCAT(tra_vc150_ape,', ',tra_vc150_nom) FROM trabajador tra WHERE tra.tra_in11_cod=dip.tra_in11_sup) AS supervisor,
            DATE_FORMAT(dic_dt_fech,'%d/%m/%Y') AS fecha, DATE_FORMAT(dic_tm_hora,'%r') AS hora 
            FROM detalle_inspeccion_calidad dip, orden_conjunto orc, orden_produccion orp, proceso pro,conjunto conj 
            WHERE dip.orc_in11_cod=orc.orc_in11_cod AND pro.pro_in11_cod=dip.pro_in11_cod AND 
            dip.ort_vc20_cod=orp.ort_vc20_cod AND orc.con_in11_cod=conj.con_in11_cod AND orp.orp_in11_numope = '$op' AND dip.pro_in11_cod = '$proc' AND dic_in1_sta != 0
            ORDER BY dic_dt_fech ASC, dic_tm_hora ASC");
        }
        return $cons;
    }
