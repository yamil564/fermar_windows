...
class CLS_ConfigArea extends PDF_addonXMP {
    ...
    function fun_Cabe() {
        ...
        /* Lista los procesos de acuerdo a la configuracion escogida hasta 8 procesos*/
        ...
        $consPro = $db->consulta("SELECT pro_in11_cod, pro_vc10_alias FROM proceso WHERE pro_in11_cod IN(" . $rowProGen['reac_vc80_pro'] . ") AND pro_in1_est != 0 ORDER BY pro_in2_ord LIMIT 0, 14");
        $x1 = 0;
        $x2 = 0;
        $x3 = 0;
        $x4 = 0;
    ...
    function CabeDetalle() {
        ...
        $consPg = $db->consulta("SELECT pro_in11_cod, pro_vc10_alias FROM proceso WHERE pro_in11_cod IN(" . $rowP['reac_vc80_pro'] . ") AND pro_in1_est != 0 ORDER BY pro_in2_ord LIMIT 0, 14");
        $xp1 = 0;
        $xp2 = 0;
        $pesoProcesao = 0;
        $totalpesoProcesar = 0;
...
$pdf = new CLS_ConfigArea("L", "mm", "A3");
$pdf->AddPage();
$pdf->AliasNbPages();
...
while ($row = $db->fetch_assoc($sqlOT)) {
    ...
    $rowProGen = $db->fetch_assoc($consProGen);
    $consPro = $db->consulta("SELECT pro_in11_cod, pro_vc10_alias FROM proceso WHERE pro_in11_cod IN(" . $rowProGen['reac_vc80_pro'] . ") AND pro_in1_est != 0 ORDER BY pro_in2_ord ASC LIMIT 0, 14");
    while ($rowProc = $db->fetch_assoc($consPro)) {
        ...
        $dot_do_peso=$arr[2];
        //echo 'Peso=> '.$p;
        if (is_numeric($dot_do_peso)) {
            $pesoCal = (($p / 100) * $dot_do_peso);
        } else {
            // Asignar un valor predeterminado o mostrar un mensaje de error
            $pesoCal = 0; // Valor predeterminado en caso de que $dot_do_peso no sea numérico
        }
