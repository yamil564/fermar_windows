...
/*
  ...
  | @Fecha de la ultima modificacion:03/06/2013
...
class CLS_Etiqueta extends PDF_Code128 {
    ...
    function etiqueta($marca, $marcli, $tipo, $base, $acab, $marco, $fierro, $cod, $posY) {
        ...
        $this->SetTextColor(23);
        $this->MultiCell(7, 5, utf8_decode('OT:'), '0', 'L', '0', false);
        ...
        $this->SetFont('Arial', 'B', 6);
        $this->SetTextColor(23);
        $this->MultiCell(32, 5, utf8_decode(strtoupper($base . '-' . $acab)), '0', 'L', '0', false);
        ...
        $this->SetFont('Arial', 'B', 8);
        $this->SetFillColor(0, 0, 0);
        $this->MultiCell(52, 4, utf8_decode(strtoupper($_REQUEST['ot'] . ' ' . $marca . '(' . $arrMarca[$countarr - 1] . ')')), '0', 'L', false);
        $this->Ln(3);
...
if (strtolower($orc) == 'all') {
    $cons = $spEtiq->SP_LisItemAll($op);
} else {
    //$cons = $spEtiq->SP_LisItem($orc, $op);
    $cons = $spEtiq->SP_LisItem($codNuevo, $op);
}
//Impreso de codigo de barras de las rejillas o peldaños
$pdf->SetY(0);
//$pdf->SetY(4);
//Formando las etiquetas
$rowCount = $db->num_rows($cons);
$item = 0;
$posY = -2;
