...
class CLSInspeccionList extends PDF_Code128 {
    ...
    function Header() {
        ...
        $this->SetTextColor(23);
        $this->MultiCell(297, 4, 'ORDEN DE PRODUCCION          GFA-For-005 VO1 30/05/2013', '', 'C', '', false);
...
#6M (6 Barra de metros) del Arrioste
    ...
    if ($res_arri['dco_do_largo'] <= 6000) {
        if ($res_arri['dco_in11_cant'] != 0) {
            $m6_arri = $res_arri['dco_do_largo'] / (floor(6000 / $res_arri['dco_in11_cant']));
        } else {
            // Asignar un valor por defecto cuando el denominador es cero
            $m6_arri = 0;
        }
        //Calculando los 6M del Portante
    } else {
