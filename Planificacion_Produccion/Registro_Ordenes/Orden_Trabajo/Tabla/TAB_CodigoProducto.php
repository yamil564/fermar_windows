...
/*
|---------------------------------------------------------------
| PHP TAB_CodigoProducto.php
|---------------------------------------------------------------
| @Autor: Frank Peña Ponce
| @Fecha de creacion: 10/09/2011
| @Fecha de la ultima modificacion: 10/09/2011
| @Organizacion: KND S.A.C.
|---------------------------------------------------------------
| Pagina donde se encuentra la estructura del JqGrid en JSON del filtro para el codigo del Producto
*/
...
$result = $db->consulta("SELECT COUNT(*) AS count FROM conjunto_base WHERE cob_in1_est != 0 $filter ");
...
$SQL = "SELECT * FROM conjunto_base WHERE cob_in1_est != 0 $filter ORDER BY $sidx $sord LIMIT $start, $limit";

$result = $db->consulta($SQL);
while($row =  $db->fetch_assoc($result)) {
    $responce->rows[$i]['id']=$row['cob_vc50_cod'];
    $responce->rows[$i]['cell']=array('',$row['cob_vc50_cod'],$row['cob_vc50_desc'],$row['cob_vc100_ali']);
        $i++;
}
