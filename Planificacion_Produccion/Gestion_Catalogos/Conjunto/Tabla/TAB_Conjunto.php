...
|---------------------------------------------------------------
| Pagina donde se encuentra la estructura del JqGrid en JSON del Conjunto
*/
...
$result = $db->consulta("SELECT COUNT(*) AS count FROM conjunto c, conjunto_base cb WHERE c.cob_vc50_cod = cb.cob_vc50_cod AND con_in1_est != '0' $filter ");
...
$SQL = "SELECT c.con_in11_cod, cb.cob_vc50_cod, c.con_vc20_nroplano, c.con_vc20_marcli, c.con_in11_cant, c.con_do_largo, c.con_do_ancho, c.con_vc11_codtipcon,con_vc50_observ
FROM conjunto c, conjunto_base cb
WHERE c.cob_vc50_cod = cb.cob_vc50_cod AND con_in1_est != '0' $filter ORDER BY $sidx $sord LIMIT $start, $limit";


$result = $db->consulta($SQL);
while($row =  $db->fetch_assoc($result)){
    $responce->rows[$i]['id']=$row['con_in11_cod'];
    $responce->rows[$i]['cell']=array('',$row['con_in11_cod'],$row['cob_vc50_cod'],$row['con_vc20_nroplano'],$row['con_vc20_marcli'],$row['con_in11_cant'],$row['con_do_largo'],$row['con_do_ancho'],$row['con_vc11_codtipcon'],$row['con_vc50_observ']);
        $i++;
};
