...
/*
|---------------------------------------------------------------
| PHP TAB_ConjuntoBase.php
|---------------------------------------------------------------
| @Autor: Kenyi Caycho Coyocusi
| @Fecha de creacion: 22/12/2010
...
|---------------------------------------------------------------
| Pagina donde se encuentra la estructura del JqGrid en JSON del Conjunto Base
*/
...
$result = $db->consulta("SELECT COUNT(*) AS count FROM conjunto_base c, tipo_acabado a WHERE c.tpa_vc4_cod = a.tpa_vc4_cod and cob_in1_est != '0' $filter ");
...
$SQL = "SELECT c.cob_vc50_cod,c.cob_vc100_ali,c.cob_vc50_desc,c.cob_vc20_super,a.tpa_vc50_desc, c.cob_do_disport , c.cob_do_disarri FROM conjunto_base c, tipo_acabado a WHERE c.tpa_vc4_cod = a.tpa_vc4_cod AND cob_in1_est != '0' $filter ORDER BY $sidx $sord LIMIT $start, $limit";

$result = $db->consulta($SQL);
while($row =  $db->fetch_assoc($result)) {
    $responce->rows[$i]['id']=$row['cob_vc50_cod'];
    $responce->rows[$i]['cell']=array('',$row['cob_vc50_cod'],$row['cob_vc100_ali'],$row['cob_vc50_desc'],$row['cob_vc20_super'],$row['tpa_vc50_desc'],$row['cob_do_disport'],$row['cob_do_disarri']);
        $i++;
}
