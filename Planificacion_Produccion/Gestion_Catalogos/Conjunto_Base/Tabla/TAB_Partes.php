...
|---------------------------------------------------------------
| Pagina donde se encuentra la estructura del JqGrid en JSON de las partes del Conjunto Base
*/
...
if(isset($_GET['usu'])){
    $codusu = $_GET['usu'];
    $SQL = "SELECT COUNT(*) AS count FROM parte p, materia m, temporal_conbase t WHERE p.par_in11_cod = t.par_in11_cod  AND m.mat_vc3_cod = t.mat_vc3_cod AND t.usu_in11_cod = '$codusu' $filter ";
}
if(isset($_GET['cod'])){
    $cod = $_GET['cod'];
...
if(isset ($_GET['cod'])){
    ...
    $SQL = "SELECT p.par_in11_cod, p.par_vc50_desc, m.mat_vc3_cod, m.mat_vc50_desc, m.mat_do_largo, m.mat_do_ancho, m.mat_do_espesor, m.mat_do_diame
            FROM materia m, parte p, detalle_conjunto_base d
                WHERE  d.par_in11_cod = p.par_in11_cod AND d.mat_vc3_cod = m.mat_vc3_cod AND cob_vc50_cod='".$cod."' $filter ORDER BY $sidx $sord LIMIT $start , $limit";
}
