...
if(isset($_GET['REQUE'])){
    $REQUE = $_GET['REQUE'];
    $SQL = "SELECT COUNT(*) AS count FROM requisicion WHERE ort_vc20_cod ='$REQUE' $filter";
}
...
if(isset($_GET['REQUE'])){
    $REQUE = $_GET['REQUE'];
    $SQL = "SELECT * FROM requisicion WHERE ort_vc20_cod = '$REQUE' ORDER BY $sidx $sord LIMIT $start, $limit";
}

$result = $db->consulta($SQL);
while($row =  $db->fetch_assoc($result)) {
 if(isset ($_GET['REQUE'])){
    $responce->rows[$i]['id']=$row['req_in11_cod'];
    $responce->rows[$i]['cell']=array($row['req_vc50_part'],$row['req_vc80_desc'],$row['req_do_min'],$row['req_do_minap'],$row['req_do_max']);
     }
    $i++;
}
