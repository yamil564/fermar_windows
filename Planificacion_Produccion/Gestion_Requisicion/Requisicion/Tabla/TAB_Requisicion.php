...
|---------------------------------------------------------------
| Pagina donde se encuentra la estructura del JqGrid en JSON de los requisiciones
*/
...
$result = $db->consulta("SELECT * FROM requisicion WHERE req_in1_pre !=0 AND req_in1_est !=0 $filter ");
...
$SQL = "SELECT * FROM requisicion WHERE req_in1_pre !=0 AND req_in1_est !=0 $filter ORDER BY $sidx $sord LIMIT $start , $limit";
$result = $db->consulta($SQL);

while($row =  $db->fetch_assoc($result)){
    $cod = $row['req_in11_cod'];
    $responce->rows[$i]['id']=$row['ort_vc20_cod'];
    $responce->rows[$i]['cell']=array('',$cod,$row['ort_vc20_cod'],$row['req_da_fech']);
        $i++;
};
