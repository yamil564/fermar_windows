...
|---------------------------------------------------------------
| Pagina donde se encuentra la estructura del JqGrid en JSON de los Planos
*/
...
$result = $db->consulta("SELECT COUNT(*) AS count FROM plano WHERE pla_in1_est != '0' $filter ");
...
$SQL = "SELECT * FROM plano WHERE pla_in1_est != '0' $filter ORDER BY $sidx $sord LIMIT $start , $limit";


$result = $db->consulta($SQL);
while($row =  $db->fetch_assoc($result)) {
    $cont = strlen($row['pla_in11_nro']);
    switch($cont){
        case 1: $cod = 'P0000000'.$row['pla_in11_nro'];break;
        case 2: $cod = 'P000000'.$row['pla_in11_nro'];break;
	case 3: $cod = 'P00000'.$row['pla_in11_nro'];break;
	case 4: $cod = 'P0000'.$row['pla_in11_nro'];break;
	case 5: $cod = 'P000'.$row['pla_in11_nro'];break;
	case 6: $cod = 'P00'.$row['pla_in11_nro'];break;
	case 7: $cod = 'P0'.$row['pla_in11_nro'];break;
        case 8: $cod = 'P'.$row['pla_in11_nro'];break;
    }
    $responce->rows[$i]['id']=$row['pla_in11_nro'];
    $responce->rows[$i]['cell']=array('',$cod,$row['ort_in11_num']);
        $i++;
}
