...
| @Fecha de la ultima modificacion: 23/12/2010
...
$result = $db->consulta("SELECT COUNT(*) AS count FROM tipo_acabado WHERE tpa_in1_est != '0' $filter ");
...
$SQL = "SELECT * FROM tipo_acabado WHERE tpa_in1_est != '0' $filter ORDER BY $sidx $sord LIMIT $start , $limit";
$result = $db->consulta($SQL);
while($row =  $db->fetch_assoc($result)){
    $cod = $row['tpa_vc4_cod'];
    $responce->rows[$i]['id']=$row['tpa_vc4_cod'];
    $responce->rows[$i]['cell']=array('',utf8_encode($cod),$row['tpa_vc50_desc'],utf8_encode($row['tpa_vc3_alias']));
        $i++;
};
