...
/*
|---------------------------------------------------------------
| PHP TAB_Proveedor.php
|---------------------------------------------------------------
| @Autor: Kenyi Caycho Coyocusi
| @Fecha de creacion: 10/12/2010
| @Organizacion: KND S.A.C.
|---------------------------------------------------------------
| Pagina donde se encuentra la estructura del JqGrid en JSON de las Proveedor
*/
...
$result = $db->consulta("SELECT COUNT(*) AS count FROM proveedor WHERE pvr_in1_est != '0' $filter ");
...
$SQL = "SELECT * FROM proveedor WHERE pvr_in1_est != '0' $filter ORDER BY $sidx $sord LIMIT $start , $limit";
$result = $db->consulta($SQL);
while($row =  $db->fetch_assoc($result)) {
    $cont = strlen($row['pvr_in11_cod']);
    /* Sentencia para Listar los proveedores concatenados */
    switch($cont){
        case 1: $cod = 'PV0000000'.$row['pvr_in11_cod'];break;
        case 2: $cod = 'PV000000'.$row['pvr_in11_cod'];break;
	case 3: $cod = 'PV00000'.$row['pvr_in11_cod'];break;
	case 4: $cod = 'PV0000'.$row['pvr_in11_cod'];break;
	case 5: $cod = 'PV000'.$row['pvr_in11_cod'];break;
	case 6: $cod = 'PV00'.$row['pvr_in11_cod'];break;
	case 7: $cod = 'PV0'.$row['pvr_in11_cod'];break;
        case 8: $cod = 'PV'.$row['pvr_in11_cod'];break;
    }
    $responce->rows[$i]['id']=$row['pvr_in11_cod'];
    $responce->rows[$i]['cell']=array('',$cod,$row['pvr_vc11_ruc'],$row['pvr_vc20_razsocial'],$row['pvr_vc150_dir']);
        $i++;
}
