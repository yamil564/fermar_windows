...
|---------------------------------------------------------------
| Pagina donde se encuentra la estructura del JqGrid en JSON del Proceso de Sub_Codigo
*/
...
$result = $db->consulta("SELECT COUNT(*) AS count FROM `proceso_sub_codigo` WHERE `psu_in11_cod`!='0' $filter ");
...
$SQL = "SELECT `psu_in11_cod`,`psu_vc20_des` FROM `proceso_sub_codigo` WHERE `psu_ch1_est`!='0' $filter ORDER BY $sidx $sord LIMIT $start , $limit";

$result = $db->consulta($SQL);
while($row =  $db->fetch_assoc($result)) {
    $responce->rows[$i]['id']=$row['psu_in11_cod'];
    $responce->rows[$i]['cell']=array('',$row['psu_in11_cod'],$row['psu_vc20_des']);
        $i++;
}
