...
|---------------------------------------------------------------
| Pagina donde se encuentra la estructura del JqGrid en JSON del Proceso de Fusion
*/
...
$result = $db->consulta("SELECT COUNT(*) AS count FROM `proceso_fusion` WHERE pfu_ch1_est !='0' $filter ");
...
$SQL = "SELECT * FROM proceso_fusion WHERE pfu_ch1_est !='0' $filter ORDER BY $sidx $sord LIMIT $start , $limit";


$result = $db->consulta($SQL);
while($row =  $db->fetch_assoc($result)) {
    $responce->rows[$i]['id']=$row['pfu_in11_cod'];
    $responce->rows[$i]['cell']=array('',$row['pfu_in11_cod'],$row['pfu_vc20_des']);
        $i++;
}
