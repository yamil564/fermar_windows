...
if($search_ind=='true'){
    ...
    foreach($filter_rul as $key => $value){
        ...
        if($fie == 'usuario'){
            $fie = "CONCAT(usu_vc80_ape,', ',usu_vc80_nom)";
        }
        if($fie == 'hora'){
           $fie = "DATE_FORMAT(reac_tm_hora, '%r')";
        }
        if($fie == 'fecha'){
           $fie = "DATE_FORMAT(reac_date_fec,'%d/%m/%Y')";
        }
        if($cont==0){
           if($opt=="REGEXP1"){
                $opt = "LIKE";
...
$result = $db->consulta("SELECT COUNT(*) AS count FROM reporte_area_cabe rac, usuario usu WHERE rac.usu_in11_cod=usu.usu_in11_cod AND reac_in1_sta != 0 $filter ");
...
$i=0;
$SQL = "SELECT reac_in11_cod, CONCAT(usu_vc80_ape,', ',usu_vc80_nom) AS usuario, reac_vc80_des, DATE_FORMAT(reac_tm_hora, '%r') AS hora, DATE_FORMAT(reac_date_fec,'%d/%m/%Y') AS fecha
        FROM reporte_area_cabe rac, usuario usu WHERE rac.usu_in11_cod=usu.usu_in11_cod AND reac_in1_sta != 0 $filter ORDER BY $sidx $sord LIMIT $start , $limit";
$result = $db->consulta($SQL);
while($row =  $db->fetch_assoc($result)) {   
    $responce->rows[$i]['id']=$row['reac_in11_cod'];
    $responce->rows[$i]['cell']=array('',$row['reac_vc80_des'],$row['usuario'],$row['hora'],$row['fecha']);
    $i++;
}
