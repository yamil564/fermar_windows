...
$result = $db->consulta("SELECT COUNT(*) AS count FROM temp_proceso WHERE usu_in11_cod = '$usu' $filter ");
...
$i=0;
$SQL = "SELECT pro_in11_cod, tmpp_vc50_desc, tmpp_in1_sta FROM temp_proceso WHERE usu_in11_cod = '$usu' $filter ORDER BY $sidx $sord LIMIT $start , $limit";
$result = $db->consulta($SQL);
while($row =  $db->fetch_assoc($result)) { 
    //echo "Dentro row";  
    $responce->rows[$i]['id']=$row['pro_in11_cod'];
    $responce->rows[$i]['cell']=array($row['tmpp_in1_sta'].'::'.$row['pro_in11_cod'], $row['tmpp_vc50_desc']);
    $i++;
}
