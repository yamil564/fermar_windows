...
/*
|---------------------------------------------------------------
| PHP TAB_Proyecto.php
|---------------------------------------------------------------
| @Autor: Kenyi Caycho Coyocusi
| @Fecha de creacion: 11/12/2010
| @Organizacion: KND S.A.C.
|---------------------------------------------------------------
| Pagina donde se encuentra la estructura del JqGrid en JSON de los Proyectos
*/
...
$result = $db->consulta("SELECT COUNT(*) AS count FROM proyecto WHERE pyt_in1_est != '0' $filter ");
...
$SQL = "SELECT * FROM proyecto WHERE pyt_in1_est != '0' $filter ORDER BY $sidx $sord LIMIT $start , $limit";
$result = $db->consulta($SQL);
while($row =  $db->fetch_assoc($result)) {
    $cont = strlen($row['pyt_in11_cod']);
    switch($cont){
        case 1: $cod = 'P0000000'.$row['pyt_in11_cod'];break;
        case 2: $cod = 'P000000'.$row['pyt_in11_cod'];break;
	case 3: $cod = 'P00000'.$row['pyt_in11_cod'];break;
        case 4: $cod = 'P0000'.$row['pyt_in11_cod'];break;
        case 5: $cod = 'P000'.$row['pyt_in11_cod'];break;
        case 6: $cod = 'P00'.$row['pyt_in11_cod'];break;
        case 7: $cod = 'P0'.$row['pyt_in11_cod'];break;
        case 8: $cod = 'P'.$row['pyt_in11_cod'];break;
    }
    $responce->rows[$i]['id']=$row['pyt_in11_cod'];
    $responce->rows[$i]['cell']=array('',$cod,$row['pyt_vc150_nom']);
        $i++;
}
