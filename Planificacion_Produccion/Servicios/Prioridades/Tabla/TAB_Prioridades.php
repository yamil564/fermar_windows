...
|---------------------------------------------------------------
| Pagina donde se encuentra la estructura del JqGrid en JSON de las Prioridadeses
*/
...
$result = $db->consulta("SELECT COUNT(*) AS count  FROM prioridades WHERE pri_in1_est != 0 $filter ");
...
$SQL = "SELECT pri_in11_cod, con_vc50_observ, pri_do_orden FROM prioridades
        WHERE pri_in1_est != 0 $filter ORDER BY $sidx $sord LIMIT $start , $limit";
$result = $db->consulta($SQL);
while($row =  $db->fetch_assoc($result)) {

    $responce->rows[$i]['id']=$row['pri_in11_cod'];
    $responce->rows[$i]['cell']=array('',$row['pri_in11_cod'],$row['con_vc50_observ'],$row['pri_do_orden']);
        $i++;
}
