...
$result = $db->consulta("SELECT COUNT(*) AS 'count' FROM componentespel cmp, parte prt WHERE cmp.par_in11_cod=prt.par_in11_cod AND cmp_in1_est != '0' $filter ");
...
$SQL = "SELECT cmp_in11_cod, cmp_vc50_des, cmp_do_pml, cmp_do_esp, par_vc50_desc
        FROM componentespel cmp, parte prt WHERE cmp.par_in11_cod=prt.par_in11_cod AND cmp_in1_est != '0' $filter ORDER BY $sidx $sord LIMIT $start , $limit";


$result = $db->consulta($SQL);
while($row =  $db->fetch_assoc($result)) {
    $responce->rows[$i]['id']=$row['cmp_in11_cod'];
    $responce->rows[$i]['cell']=array('',$row['cmp_in11_cod'],$row['cmp_vc50_des'],$row['cmp_do_pml'],$row['cmp_do_esp'],$row['par_vc50_desc']);
    $i++;
}
