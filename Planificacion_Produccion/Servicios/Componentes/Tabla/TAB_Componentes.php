...
|---------------------------------------------------------------
| Pagina donde se encuentra la estructura del JqGrid en JSON del los Componentes
*/
...
$result = $db->consulta("SELECT COUNT(*) AS 'count' FROM componentes c, parte p WHERE c.par_in11_cod=p.par_in11_cod AND com_in1_est != '0' $filter ");
...
$SQL = "SELECT com_vc10_cod, com_vc150_desc, com_do_espes, com_do_pesoml, com_do_pesom2, par_vc50_desc
        FROM componentes c, parte p WHERE c.par_in11_cod=p.par_in11_cod AND com_in1_est != '0' $filter ORDER BY $sidx $sord LIMIT $start , $limit";


$result = $db->consulta($SQL);
while($row =  $db->fetch_assoc($result)) {
    $responce->rows[$i]['id']=$row['com_vc10_cod'];
    $responce->rows[$i]['cell']=array('',$row['com_vc10_cod'],$row['com_vc150_desc'],$row['com_do_pesoml'],$row['com_do_pesom2'],$row['par_vc50_desc']);
    $i++;
}
